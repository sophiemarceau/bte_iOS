...
@implementation SharedDugView

/**
 分享弹窗
 */
+ (void)popShareViewCallBack:(ShareDigCallBack)shareViewCallBack
                    imageUrl:(id)_imageUrl shareUrl:(NSString *)_shareUrl sharetitle:(NSString *)_sharetitle shareDesc:(NSString *)_shareDesc shareType:(UMSHAREPic_TYPE)_shareType currentVc:(UIViewController *)currentVc WithQrImage:(UIImage *)erCodeImage WithInviteCode:(NSString *)inviteCodeStr WithDugNum:(NSString *)dugNumStr{
    SharedDugView *shareView = [[SharedDugView alloc] initWithFrame:[UIScreen mainScreen].bounds];
    if(shareView){
    ...
    [UIImage imageNamed:@"挖矿ShareBgView"];
    ...
    UIImage *targetImage = [shareView imageCompressFitSizeScale:[shareView colorForImage:erCodeImage red:69/255.0f green:47/255.0f blue:117/255.0f] targetSize:CGSizeMake(248, 248)];
    ...
    qrimageView.frame = CGRectMake((bgView.width -248)/2, bgView.height - 248  - 264, 248, 248);
    [bgView addSubview:qrimageView];
    
    
//    if (!IsNilOrNull(dugNumStr)) {
//        UILabel *inviteLabel = [[UILabel alloc] init];
//        inviteLabel.textAlignment =NSTextAlignmentCenter;
//        inviteLabel.font = UIFontMediumOfSize(20);
//        inviteLabel.textColor = BHHexColor(@"FFFFF");
//
//        NSMutableAttributedString *string = [[NSMutableAttributedString alloc] initWithString: [NSString stringWithFormat:@"我是第%@位矿工，正在瓜分价值五百万BTC",dugNumStr]];
//        [string addAttribute:NSForegroundColorAttributeName value:BHHexColor(@"308CDD")range:NSMakeRange(3, dugNumStr.length)];
//        inviteLabel.attributedText = string;
//        inviteLabel.frame = CGRectMake(0, 30, bgView.width, 20);
...
//    }
    ...
    if (!IsNilOrNull(inviteCodeStr)) {
        UILabel *inviteLabel = [[UILabel alloc] init];
        inviteLabel.textAlignment =NSTextAlignmentCenter;
        inviteLabel.font = UIFontSemiboldOfSize((52));
        inviteLabel.textColor = BHHexColor(@"7259D0");
        inviteLabel.text = inviteCodeStr;
        inviteLabel.frame = CGRectMake(0, 550,bgView.width , (52));
        [bgView addSubview:inviteLabel];
    }
...
-(UIImage *)colorForImage:(UIImage *)erImage red:(CGFloat)red green:(CGFloat)green blue:(CGFloat)blue{
    int imageWidth = erImage.size.width;
    int imageHeight = erImage.size.height;
    size_t bytesPerRow = imageWidth * 4;
    uint32_t *rgbImageBuf = (uint32_t *)malloc(bytesPerRow * imageHeight);
    CGColorSpaceRef colorSpaceRef = CGColorSpaceCreateDeviceRGB();
    CGContextRef context = CGBitmapContextCreate(rgbImageBuf, imageWidth, imageHeight, 8, bytesPerRow, colorSpaceRef, kCGBitmapByteOrder32Little|kCGImageAlphaNoneSkipLast);
    CGContextDrawImage(context, CGRectMake(0, 0, imageWidth, imageHeight), erImage.CGImage);
    //遍历像素, 改变像素点颜色
...
- (void)confirmbuttondismiss:(UIButton *)sender {
    ...
    if (self.shareViewCallBack) {
        self.shareViewCallBack();
        [self removeFromSuperview];
    }
}

- (void)shareWithType:(UMSHAREPic_TYPE)type{
    switch (type) {
        case UMSHAREPic_TYPE_TEXT:
        {
            [self shareTextToPlatformType:self.platform];
        }
            break;
        case UMSHAREPic_TYPE_IMAGE:
        {
            [self shareImageToPlatformType:self.platform];
        }
            break;
        case UMSHAREPic_TYPE_IMAGE_URL:
        {
            [self shareImageURLToPlatformType:self.platform];
        }
            break;
        case UMSHAREPic_TYPE_TEXT_IMAGE:
        {
            [self shareImageAndTextToPlatformType:self.platform];
        }
            break;
        case UMSHAREPic_TYPE_WEB_LINK:
        {
...
//    }
//
//    methodName = kDigShare;
//
//    NSLog(@"pramaDic----->%@",pramaDic);
//    NMShowLoadIng;
//    [BTERequestTools requestWithURLString:methodName parameters:pramaDic type:2 success:^(id responseObject) {
//         NSLog(@"kDigShare----->%@",responseObject);
//        NMRemovLoadIng;
//        if (IsSafeDictionary(responseObject)) {
//        }
//    } failure:^(NSError *error) {
