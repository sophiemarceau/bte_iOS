...
@interface ChatViewController ()<EaseMessageViewControllerDelegate,EaseMessageViewControllerDataSource,EMClientDelegate,EMChatManagerDelegate,EMChatroomManagerDelegate,ZTYChartProtocol>
...
- (instancetype)initBase:(NSString *)base groupName:(NSString *)groupName quote:(NSString *)quote exchange:(NSString *)exchange chatRoomStr:(NSString *)chatRoomStr{
    if (self = [super init]) {
...
- (void)viewDidLoad {
    ...
    // ****************K线*************//
    self.kdataSource = [NSMutableArray array];
    _ktype = @"1h";
    _isrelaod = 0;
...
//    [self createHeadView];
    ...
    [[EaseBaseMessageCell appearance] setSendBubbleBackgroundImage:[[UIImage imageNamed:@"kchat_sender_bg"] stretchableImageWithLeftCapWidth:15 topCapHeight:10]];
    [[EaseBaseMessageCell appearance] setMessageTextFont:[UIFont fontWithName:@"PingFangSC-Regular" size:14]];
    [[EaseBaseMessageCell appearance] setRecvBubbleBackgroundImage:[[UIImage imageNamed:@"kchat_receiver_bg"] stretchableImageWithLeftCapWidth:15 topCapHeight:10]];
    self.showRefreshHeader = YES;
    self.delegate = self;
    self.dataSource = self;
    self.tableView.backgroundColor = BHHexColor(@"EDF0F2");
    ...
    self.topImageView.frame = CGRectMake(SCREEN_WIDTH-SCALE_W(28)-SCALE_W(10), SCREEN_HEIGHT - 240, SCALE_W(28), SCALE_W(28));
...
-(void)gotoTop{
    [self.chatdelegate gotoSuperViewTop];
}
...
- (void)setChatToolbar:(EaseChatToolbar *)chatToolbar{
    [super setChatToolbar:chatToolbar];
    [self.chatBarMoreView removeItematIndex:1];//在更多里面 删除发送位置的 按钮
    [self.chatBarMoreView updateItemWithImage:[UIImage imageNamed:@"BTEchatBar_colorMore_camera"] highlightedImage:[UIImage imageNamed:@"BTEchatBar_colorMore_camera"] title:@"相机" atIndex:0];
    [self.chatBarMoreView updateItemWithImage:[UIImage imageNamed:@"BTEchatBar_colorMore_photo"] highlightedImage:[UIImage imageNamed:@"BTEchatBar_colorMore_photo"] title:@"相册" atIndex:1];
}
...
- (void)GetRoomInfo{
    ...
    NSString * methodName = @"";
    [pramaDic setObject:self.base forKey:@"base"];
    ...
    [BTERequestTools requestWithURLString:methodName parameters:pramaDic type:2 success:^(id responseObject) {
        ...
        if (IsSafeDictionary(responseObject)) {
             weakSelf.title = [NSString stringWithFormat:@"%@行情讨论(%@)",self.base,responseObject[@"data"][@"result"]];
...
- (void)messagesDidReceive:(NSArray *)aMessages
{
    ...
    for(EMMessage *message in aMessages){
        if (message.ext) {
            ...
            [UserCacheManager getUserInfo:model.nickname];
        }
...
{
    message.ext = [UserCacheManager getMyMsgExt:message.ext];
                           ...
                           modelForMessage:(EMMessage *)message
{
    ...
    [UserCacheManager getUserInfo:model.nickname completed:^(UserCacheInfo *userInfo) {
            if (userInfo) {
                            ...
                            if (model) {
                                ...
                                [self.dataArray enumerateObjectsUsingBlock:^(EaseMessageModel *model, NSUInteger idx, BOOL *stop){
                                    if ([model conformsToProtocol:@protocol(IMessageModel)]) {
                                        if ([message.messageId isEqualToString:model.message.messageId])
                                        {
                                            index = idx;
                                            *stop = YES;
                                        }
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    [self GetRoomInfo];
//    self.navigationController.navigationBar.titleTextAttributes = @{NSForegroundColorAttributeName:BHHexColor(@"#454F6B"), NSFontAttributeName : UIFontRegularOfSize(13)};
}
...
- (void)addHiddeView{
    ...
    self.headView.layer.shadowColor = [UIColor colorWithHexString:@"6F7D8A" alpha:0.1].CGColor;
    self.headView.layer.shadowOffset = CGSizeMake(10, 10);
    self.headView.layer.shadowOpacity = 1;
...
//
//    }
    CGFloat timeBtnW = SCREEN_WIDTH / (self.klineTypes.count * 1.0);
    UIView * btnLine = [[UIView alloc] initWithFrame:CGRectMake((timeBtnW - 16) * 0.5, 138, 16, 2)];
    btnLine.backgroundColor = [UIColor colorWithHexString:@"308cdd"];
    btnLine.tag = 999;
    [self.KlineBgView addSubview:btnLine];
    for (int index = 0; index < self.klineTypes.count; index ++) {
        
        UIButton * btn = [self createBtn:self.klineTypes[index] frame:CGRectMake(index * timeBtnW, 120, timeBtnW, 18)];
        [btn addTarget:self action:@selector(choseTimeItem:) forControlEvents:UIControlEventTouchUpInside];
        btn.tag = 4000 + index;
        [self.KlineBgView addSubview:btn];
        
    }
    
    UIButton * foldBtn = [[UIButton alloc] initWithFrame:CGRectMake(SCREEN_WIDTH - 16 - 20, 0, 20, 20)];
    foldBtn.tag = 200;
...
- (void)choseTimeItem:(UIButton *)btn{
    for (int i = 0; i < self.klineTypes.count; i ++) {
        UIButton * button = [self.KlineBgView viewWithTag:4000 + i];
    ...
    CGFloat timeW = SCREEN_WIDTH / (self.klineTypes.count * 1.0);
    UIView * line = [self.KlineBgView viewWithTag:999];
    line.frame = CGRectMake(timeW * (btn.tag - 4000) + (timeW - 16) * 0.5, 138, 16, 2);
    
    NSArray * types= @[@"1m",@"1m",@"5m",@"15m",@"30m",@"1h",@"4h",@"1d",@"1w",@"1mo"];
    _ktype = types[btn.tag -4000];
...
- (void)addBGview{
    ...
    self.KlineBgView.layer.shadowColor = [UIColor colorWithHexString:@"6F7D8A" alpha:0.1].CGColor;
    self.KlineBgView.layer.shadowOffset = CGSizeMake(10, 10);
    self.KlineBgView.layer.shadowOpacity = 1;
    self.KlineBgView.layer.shadowRadius = 5;
}
...
- (void)addChartTitle{
    ...
    UILabel * riseDropLabel = [self createLabelTitle:@"涨幅：+0.27%  振幅：0.44%" frame:CGRectMake(0,  12, SCREEN_WIDTH - 32, 10)];
    ...
    _mainTextLabel = [self createLabelTitle:@"" frame:CGRectMake(0, 12, SCREEN_WIDTH - 62, 10)];
...
//            NSLog(@"responseObject======>%@",responseObject);
            ...
            if (self.kdataSource.count > 0 && arry.count > 0 ) {
                ZTYChartModel * first = [self.kdataSource firstObject];
                ZTYChartModel * last = [arry lastObject];
                first.preKlineModel = last;
            }
            [arry addObjectsFromArray:self.kdataSource];
            self.kdataSource = arry.mutableCopy;
            
            NSArray *QuotaDataArray = [[ZXQuotaDataReformer sharedInstance] initializeQuotaDataWithArray:self.kdataSource];
            [weakSelf reloadData:QuotaDataArray reload:_isrelaod];
            
            
            if (self.kdataSource.count > 0) {
                ZTYChartModel * lastModel = [self.kdataSource lastObject];
...
//    NSLog(@"requestLastModelData:param====>%@",param);
    ...
    [BTERequestTools requestWithURLString:kGetKLineData parameters:param type:1 success:^(id responseObject) {
        ...
        if (IsSafeDictionary(responseObject)) {
            ...
            __block ZTYChartModel * preLastModel = [self.kdataSource lastObject];
...
- (void)updateLatestTextLine{
    ...
    [self.latestLinetextView mas_updateConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(pointY - 7.5);
    }];
    //    }
}
