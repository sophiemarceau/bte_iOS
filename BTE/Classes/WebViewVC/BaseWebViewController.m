...
- (void)viewDidLoad {
    ...
    [self.view addSubview:self.rootKScrollView];
...
-(void)setIsHiddenBottom:(BOOL)isHiddenBottom{
    _webView.frame = CGRectMake(0,0,SCREEN_WIDTH,SCREEN_HEIGHT- NAVIGATION_HEIGHT  - (_isHiddenBottom ? HOME_INDICATOR_HEIGHT : TAB_BAR_HEIGHT));
}
...
- (void)loadWebView {
    ...
    [self.rootKScrollView addSubview:self.webView];
}
...
//static BOOL first=YES;//定义一个静态变量，判断是否是第二次加载
...
- (void)handlerNetworkUnReachableStatus {
    ...
    [[AFNetworkReachabilityManager sharedManager] setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        if (status == AFNetworkReachabilityStatusNotReachable || status ==AFNetworkReachabilityStatusUnknown) {
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:nil message:kNotReachableTipMessage delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
            [alert show];
        }
    }];
}
...
-(UIScrollView *)rootKScrollView{
    if (!_rootKScrollView) {
        _rootKScrollView = [[UIScrollView alloc] initWithFrame:self.view.bounds];
    }
    return _rootKScrollView;
}
