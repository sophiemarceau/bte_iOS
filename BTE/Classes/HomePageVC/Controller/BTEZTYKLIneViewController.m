//
//  BTEZTYKLIneViewController.m
//  BTE
//
//  Created by wanmeizty on 2018/6/19.
...
@interface BTEZTYKLIneViewController ()
...
@implementation BTEZTYKLIneViewController
...
- (void)viewDidLoad
{
    ...
    self.ktype = @"1h";
    //    _type = MACD;
    self.ChartHeight = (DEVICE_HEIGHT - headerHeight - NAVIGATION_HEIGHT - 40)* ScrollScale;
...
//                                                     Frame:CGRectMake(SCREEN_WIDTH - 36 - 12, SCREEN_HEIGHT - 36 - 67 -TAB_BAR_HEIGHT, 36, 36)
//                                                     title:@""
//                                                titleColor:[UIColor whiteColor]
//                                                 titleFont:[UIFont systemFontOfSize:11]
//                                           backgroundColor:nil
//                                           backgroundImage:[UIImage imageNamed:@"fuceng"]];
...
-(void)p_clickBtn{
    ...
    BTEKlineChatViewController * chatVC = [[BTEKlineChatViewController alloc] init];
    chatVC.base = self.base;
    chatVC.quote = self.quote;
    chatVC.exchange = self.exchange;
    [self.navigationController pushViewController:chatVC animated:YES];
    //    UIButton * btn = [[UIButton alloc] initWithFrame:CGRectMake(SCREEN_WIDTH - 60, SCREEN_HEIGHT - 80 - NAVIGATION_HEIGHT, 40, 40)];
...
- (void)addHeaderView{
    ...
    for (int i = 0; i < 3; i ++) {
        ...
        UIButton * btn = [self createBtnTiltle:arr[i] frame:CGRectMake((KbtnWidth + intalval) * i + 16, 90, KbtnWidth, 28)];
...
- (UIView *)ktypeBGView{
    if (!_ktypeBGView) {
        ...
        _ktypeBGView = [[UIView alloc] initWithFrame:CGRectMake(16, HeadTopHeight, (KbtnWidth + 10)* 3, 35 * 4)];
...
- (UIView *)mainQuotaTypeBGView{
    if (!_mainQuotaTypeBGView) {
        ...
        _mainQuotaTypeBGView = [[UIView alloc] initWithFrame:CGRectMake(16 + (KbtnWidth + intalval), HeadTopHeight, KbtnWidth, quotaArr.count * btnH)];
...
-(UIView *)fitureQuotaBGView{
    if (!_fitureQuotaBGView) {
        ...
        _fitureQuotaBGView = [[UIView alloc] initWithFrame:CGRectMake(16 + (KbtnWidth + intalval) * 2, HeadTopHeight, KbtnWidth, quotaArr.count * btnH)];
...
- (void)requestLastModelData{
    ...
    [BTERequestTools requestWithURLString:kGetKLineData parameters:param type:1 success:^(id responseObject) {
        ...
        if (IsSafeDictionary(responseObject)) {
            ...
            NSArray *QuotaDataArray = [[ZXQuotaDataReformer sharedInstance] initializeQuotaLastDataWithArray:self.dataSource];
...
//
//    if (self.dataSource.count > 0) {
//        ZYWCandleModel * first = [self.dataSource firstObject];
//        param[@"startTime"] = first.timestam;
//
//        ZYWCandleModel * last = [self.dataSource lastObject];
//        param[@"endTime"] = last.timestam;
//
//    }
//
//    WS(weakSelf)
    ...
    [BTERequestTools requestWithURLString:kGetKlineCommentList parameters:param type:2 success:^(id responseObject) {
        if (IsSafeDictionary(responseObject)) {
            ...
            if (list.count > 0) {
                ...
                for (ZTYKlineComment * comment in commentarr) {
                    for (ZTYChartModel * model in weakSelf.candleChartView.dataArray) {
                ...
                weakSelf.candleChartView.isNotComment = NO;
                [weakSelf.candleChartView reloadAtCurrentIndex];
            }else{
                _combtn.hidden = YES;
            }
        }
...
{
    _candleChartView = [ZTYMainChartView new];
...
- (UIWebView *)webView{
    if (!_webView) {
        _webView = [[UIWebView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT - NAVIGATION_HEIGHT - 40)];
