...
- (void)creatUI
{
    ...
    for (NSInteger i = 0; i < 3; i++) {
        BTEMarketNewsView *imageView = [[BTEMarketNewsView alloc] initViewWithFrame:CGRectMake(view_WIDTH * i, 1, view_WIDTH,view_HEIGHT)];
        imageView.frame = CGRectMake(view_WIDTH * i, 1, view_WIDTH,view_HEIGHT);
...
- (void)imageViewClicked:(UITapGestureRecognizer *)tap
{
    ...
    CGRect rect = [tempModel.content boundingRectWithSize:CGSizeMake(SCREEN_WIDTH - 26 * 2 - 32, 1000) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:UIFontLightOfSize(13)} context:nil];
    ...
    } else //收起
    {
        ...
        for (HomeProductModel *tempModels in _imagesArray) {
            if (tempModels.isShow) {
                CGRect rect = [tempModels.content boundingRectWithSize:CGSizeMake(SCREEN_WIDTH - 26 * 2 - 32, 1000) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:UIFontLightOfSize(13)} context:nil];
                if (currentHeight < height + rect.size.height) {
                    currentHeight = height + rect.size.height;
                }
                flag = YES;//表示至少有一个展开
