...
- (instancetype)initWithFrame:(CGRect)frame{
    if (self = [super initWithFrame:frame]) {
        ...
        [plusBtn setBackgroundImage:[UIImage imageNamed:@"工具"] forState:UIControlStateNormal];
        [plusBtn setBackgroundImage:[UIImage imageNamed:@"工具"] forState:UIControlStateHighlighted];
...
//    self.plusBtn.centerY = self.height * 0.5 - 2*LBMagin ;
    ...
    self.plusBtn.frame = CGRectMake(([UIScreen mainScreen].bounds.size.width -  self.plusBtn.size.width)/2.0, -  self.plusBtn.size.height/2.0 + 5,  self.plusBtn.size.width,  self.plusBtn.size.height);
    UILabel *label = [[UILabel alloc] init];
    label.text = @"工具";
    label.textAlignment = NSTextAlignmentCenter;
    label.frame = CGRectMake((self.width - self.width/5)/2, 35, self.width/5, 13);
    label.font = [UIFont fontWithName:@"PingFangSC-Regular" size:10];
//    label.font = [UIFont systemFontOfSize:10];
    
    //    [UIFont systemFontOfSize:11];
    //    [label sizeToFit];
    label.textColor = BHHexColor(@"ABB5BE");
    [self addSubview:label];

//    label.centerY = CGRectGetMaxY(self.plusBtn.frame) + LBMagin + 4;
