...
CGFloat const BTEMBProgressMaxOffset = 1000000.f;

static const CGFloat BTEMBDefaultPadding = 4.f;
static const CGFloat BTEMBDefaultLabelFontSize = 16.f;
static const CGFloat BTEMBDefaultDetailsLabelFontSize = 12.f;


@interface BTEMBProgressHUD ()
...
@interface BTEMBProgressHUDRoundedButton : UIButton
@end


@implementation BTEMBProgressHUD

#pragma mark - Class methods

+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    BTEMBProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    BTEMBProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (BTEMBProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            BTEMBProgressHUD *hud = (BTEMBProgressHUD *)subview;
...
- (void)commonInit {
    ...
    _animationType = BTEMBProgressHUDAnimationFade;
    _mode = BTEMBProgressHUDModeIndeterminate;
    _margin = 20.0f;
...
- (void)animateIn:(BOOL)animatingIn withType:(BTEMBProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == BTEMBProgressHUDAnimationZoom) {
        type = animatingIn ? BTEMBProgressHUDAnimationZoomIn : BTEMBProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == BTEMBProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == BTEMBProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == BTEMBProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == BTEMBProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
...
- (void)done {
    ...
    BTEMBProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<BTEMBProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    BTEMBBackgroundView *backgroundView = [[BTEMBBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = BTEMBProgressHUDBackgroundStyleSolidColor;
    ...
    BTEMBBackgroundView *bezelView = [BTEMBBackgroundView new];
    ...
    label.font = [UIFont boldSystemFontOfSize:BTEMBDefaultLabelFontSize];
    label.opaque = NO;
    ...
    detailsLabel.font = [UIFont boldSystemFontOfSize:BTEMBDefaultDetailsLabelFontSize];
    detailsLabel.opaque = NO;
    detailsLabel.backgroundColor = [UIColor clearColor];
    _detailsLabel = detailsLabel;
    
    UIButton *button = [BTEMBProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
    button.titleLabel.textAlignment = NSTextAlignmentCenter;
    button.titleLabel.font = [UIFont boldSystemFontOfSize:BTEMBDefaultDetailsLabelFontSize];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[BTEMBRoundProgressView class]];
    
    BTEMBProgressHUDMode mode = self.mode;
    if (mode == BTEMBProgressHUDModeIndeterminate) {
    ...
    else if (mode == BTEMBProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[BTEMBBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == BTEMBProgressHUDModeDeterminate || mode == BTEMBProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[BTEMBRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == BTEMBProgressHUDModeAnnularDeterminate) {
            [(BTEMBRoundProgressView *)indicator setAnnular:YES];
        }
    }
    else if (mode == BTEMBProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == BTEMBProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
- (void)updateViewsForColor:(UIColor *)color {
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[BTEMBProgressHUD class], nil];
#else
        // For iOS 9+
    ...
    } else if ([indicator isKindOfClass:[BTEMBRoundProgressView class]]) {
        BTEMBRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [BTEMBRoundProgressView appearanceWhenContainedIn:[BTEMBProgressHUD class], nil];
#else
        appearance = [BTEMBRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[BTEMBProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((BTEMBRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((BTEMBRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[BTEMBBarProgressView class]]) {
        BTEMBBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [BTEMBBarProgressView appearanceWhenContainedIn:[BTEMBProgressHUD class], nil];
#else
        ...
        if (appearance.progressColor == nil) {
            ((BTEMBBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((BTEMBBarProgressView *)indicator).lineColor = color;
        }
    } else {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
    BTEMBBackgroundView *bezelView = self.bezelView;
...
- (void)updatePaddingConstraints {
    ...
    [self.paddingConstraints enumerateObjectsUsingBlock:^(NSLayoutConstraint *padding, NSUInteger idx, BOOL *stop) {
        ...
        padding.constant = (firstVisible && (secondVisible || hasVisibleAncestors)) ? BTEMBDefaultPadding : 0.f;
...
- (void)setMode:(BTEMBProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == BTEMBProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
@implementation BTEMBRoundProgressView
...
@implementation BTEMBBarProgressView
...
@implementation BTEMBBackgroundView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_7_0) {
            _style = BTEMBProgressHUDBackgroundStyleBlur;
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
            ...
            _style = BTEMBProgressHUDBackgroundStyleSolidColor;
...
- (void)setStyle:(BTEMBProgressHUDBackgroundStyle)style {
    if (style == BTEMBProgressHUDBackgroundStyleBlur && kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNumber_iOS_7_0) {
        style = BTEMBProgressHUDBackgroundStyleSolidColor;
    }
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
...
- (void)updateForBackgroundStyle {
    BTEMBProgressHUDBackgroundStyle style = self.style;
    if (style == BTEMBProgressHUDBackgroundStyleBlur) {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == BTEMBProgressHUDBackgroundStyleBlur) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
            self.backgroundColor = self.color;
        } else {
#if !TARGET_OS_TV
...
@implementation BTEMBProgressHUDRoundedButton
