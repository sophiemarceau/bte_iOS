...
{
    if(!_EMA7) {
        _EMA7 = @((self.Close.floatValue + 6.0 * self.PreviousKlineModel.EMA7.floatValue)/8.0);
    }
    return _EMA7;
}
...
{
    if(!_EMA30) {
        _EMA30 = @((2 * self.Close.floatValue + 29.0 * self.PreviousKlineModel.EMA30.floatValue)/31.0);
    }
    return _EMA30;
}
