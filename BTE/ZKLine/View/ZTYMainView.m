...
@property (strong,nonatomic) NSString * currentShowTimestamp;
@end

@implementation ZTYMainView
...
- (void)drawCandleSublayers
{
    ...
    for (int index = 0; index < self.currentPostionArray.count; index ++) {
        ...
        ZTYChartModel *candelmodel = [self.currentDisplayArray objectAtIndex:idx];
        ...
        if (self.isNotComment == NO) {
            [self drawCommont:candelmodel positionModel:model];
        }
        
    }
}

- (void)drawCommont:(ZTYChartModel *)model positionModel:(ZTYCandlePosionModel *)posModel{
    ...
    if (model.comment != nil) {
        ...
        if (self.currentShowTimestamp == nil) {
            dialogLayer.hidden = NO;
            self.currentShowTimestamp = model.timestamp;
        }else{
            if ([self.currentShowTimestamp isEqualToString:model.timestamp]){
                dialogLayer.hidden = NO;
            }else{
        ...
        if ( layer.tag == 8888) {
            ...
            for (ZTYDialogTextLayer * dialogLayer in self.commentLayerArray) {
                ...
                if (dialogLayer.tag == layer.timestanp) {
                    ...
                    if ([self.currentShowTimestamp integerValue] == layer.timestanp) {
                        dialogLayer.hidden = YES;
                        self.currentShowTimestamp = @"0000";
                    }else{
                        dialogLayer.hidden = NO;
                        self.currentShowTimestamp = [NSString stringWithFormat:@"%ld",layer.timestanp];
                    }
