...
@interface ZTYChartView ()
<UIScrollViewDelegate,UIGestureRecognizerDelegate,UITableViewDelegate,UITableViewDataSource>
...
@property (nonatomic,assign) BOOL isHiddenDialog;
...
- (void)initLayer
{
    ...
    {
        for (NSInteger i = 0 ; i < self.commentBGLayer.sublayers.count; i++)
        {
            id layer = self.commentBGLayer.sublayers[i];
            [CATransaction begin];
            [CATransaction setDisableActions:YES];
            [layer removeFromSuperlayer];
            layer = nil;
    ...
    {
        for (NSInteger i = 0 ; i < self.commentDialogBGLayer.sublayers.count; i++)
        {
            id layer = self.commentDialogBGLayer.sublayers[i];
            [CATransaction begin];
            [CATransaction setDisableActions:YES];
            [layer removeFromSuperlayer];
            layer = nil;
...
- (void)drawCandleSublayers
{
    ...
    for (int index = 0; index < self.currentPostionArray.count; index ++) {
        ...
        ZTYChartModel * candleModel = self.currentDisplayArray[idx];
        if (!self.isNotComment && candleModel.comment) {
            [self drawCommentLayer:candleModel index:idx];
        }
    }
...
- (void)drawTimeLineLayer
{
    ...
    UIBezierPath *timePath =  [UIBezierPath drawcandlePosionLine:self.currentPostionArray withKey:@"closePoint" dayCount:0];
...
- (void)calculatePosion{
    ...
    NSUInteger idx = 0;
    for (ZTYChartModel *model in self.currentDisplayArray) {
        CGFloat xPosition = self.leftPostion + ((self.candleWidth  + self.candleSpace) * idx) + self.candleWidth/2.0;
...
- (void)showCommentBtns{
    ...
    if (!_isNotComment) {
        ...
        for (int i = 0; i < self.currentDisplayArray.count; i ++) {
            NSInteger kindex = self.currentDisplayArray.count - 1 - i;
            ZTYChartModel * candleModel = [self.currentDisplayArray objectAtIndex:kindex];
            if (candleModel.comment) {
                [self drawCommentLayer:candleModel index:i];
            }
...
- (void)drawCommentLayer:(ZTYChartModel *)model index:(NSInteger)index{
    
    ZTYDialogTextLayer * dialogLayer = [ZTYDialogTextLayer layer];
    
    if (!_currentTimestamp) {
        dialogLayer.hidden = NO;
        _currentTimestamp = [model.timestamp integerValue];
        _isHiddenDialog = NO;
    }else{
        if (_currentTimestamp == [model.timestamp integerValue]) {
            
            dialogLayer.hidden = _isHiddenDialog;
        }else{
            dialogLayer.hidden = YES;
        }
        
    }
    dialogLayer.tag = [model.comment.klineDateTime integerValue];
    CGFloat xpoint = model.markLeft + self.candleWidth/2.0;
    
    CGFloat yMaxPosition = ((self.maxY - model.low) *self.scaleY) + self.topMargin;
    // 标记位置1：上影线0：下影线  markerPlace
    if (model.comment.markerPlace == 1) {
    ...
    CGSize size = [model.comment.content getSizeOfString:[UIFont systemFontOfSize:10] constroSize:CGSizeMake(129 - 15, SCREEN_HEIGHT)];
    [dialogLayer setBackFrame:CGRectMake(xpoint -129 + 3.2 + 7.3, yMaxPosition + 19 + 2, 129, size.height + 22) contentText:model.comment.content];
...
- (void)hideDialogs{
    for (ZTYDialogTextLayer * layer in self.commentLayerArray) {
        if (layer.tag == _currentTimestamp) {
            _isHiddenDialog = YES;
            layer.hidden = _isHiddenDialog;
        }
    }
        ...
        if ( layer.tag == 8888) {
            ...
            for (ZTYDialogTextLayer * dialogLayer in self.commentLayerArray) {
                if (dialogLayer.tag == layer.timestanp ) {
                    ...
                    _currentTimestamp = layer.timestanp;
                    _isHiddenDialog = !_isHiddenDialog;
                    dialogLayer.hidden = _isHiddenDialog;
    ...
    for (NSInteger index = startIndex > 0 ? startIndex - 1 : 0; index < arrCount; ++index) {
        ...
        if(localPostion > minX && localPostion < maxX)
        {
            if(self.delegate && [self.delegate respondsToSelector:@selector(tapCandleViewWithIndex:kLineModel:startIndex:price:)])
            {
                ...
                [self judgeDialogShow:candleModel position:postion];
...
- (void)judgeDialogShow:(ZTYChartModel *)model position:(CGPoint)position{
        ...
        if (model.comment) {
            ...
            for (ZTYDialogTextLayer * dialogLayer in self.commentLayerArray) {
                ...
                if (dialogLayer.tag == [model.timestamp integerValue] ) {
                    ...
                    _currentTimestamp = dialogLayer.tag;
                    _isHiddenDialog = !_isHiddenDialog;
                    dialogLayer.hidden = _isHiddenDialog;
                }else{
                    dialogLayer.hidden = YES;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ZTYKlineTableViewCell * cell = [tableView dequeueReusableCellWithIdentifier:@"kline"];
    if (!cell) {
        cell = [[ZTYKlineTableViewCell alloc] initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:@"kline"];
    }
    ZTYChartModel * model = [_dataArray objectAtIndex:indexPath.row];
    [cell configWidth:model scaleY:self.scaleY topMargin:self.topMargin maxY:self.maxY candleWidth:self.candleWidth candleSpace:self.candleSpace];
    return cell;
}

- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    return _candleSpace + _candleWidth;
}
...
-(UITableView *)tableView{
    if (!_tableView) {
        ...
        //旋转view
        [_tableView setTransform:CGAffineTransformMakeRotation(-M_PI/2)];
    }
    return _tableView;
}
