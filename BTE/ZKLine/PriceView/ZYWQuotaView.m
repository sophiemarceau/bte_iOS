...
-(void)addConstraints
{
    ...
    [_openLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.equalTo(_highLabel.mas_bottom).offset(5);
    }];
    
    [_closeLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.equalTo(_lowLabel);
        make.top.equalTo(_lowLabel.mas_bottom).offset(5);
    }];
}

-(void)setModel:(ZTYChartModel *)model
{
    _model = model;
